...
class Predictor(object):
    ...
    def __init__(self):
        with open('data/rf.pkl', 'rb') as pkl_file:
            self.rf = pickle.load(pkl_file)
        self.weather_features = ['temp_high', 'temp_low',
                                 'hum_avg', 'wind_avg', 'vis_avg']
        self.model_features = ['year', 'month', 'DOW', 'DOY', 'temp_high', 'temp_low',
    ...
    def processing(self, data):
        ...
        for day in data[self.weather_features].values:
            p_weather_diff.append(
                self.get_p_weather_diff(day))
    ...
    def get_p_weather_diff(self, day):
        diff = np.sum(
            np.abs(np.subtract(day, self.perfect_weather) / self.perfect_weather))
        return diff
