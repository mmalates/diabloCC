...
if __name__ == '__main__':
    ...
    # remove tournament days and closures
    days = df.date.values
    ...
    df_no_zero_or_tourney.loc[df_no_zero_or_tourney.prec ==
                              'T', 'prec'] = 0.001
    df = df.fillna(0)
    ...
    fig, ax = plt.subplots(figsize=(40, 6))
    ax.plot(df_no_zero_or_tourney.date.values,
            temp_residual, label='temperature')
    ax.plot(df_no_zero_or_tourney.date.values, rounds_residual, label='rounds')
    plt.legend()
